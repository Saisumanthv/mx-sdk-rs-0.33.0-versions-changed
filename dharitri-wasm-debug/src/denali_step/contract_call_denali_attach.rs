...
use denali::model::{ScCallStep, ScDeployStep, ScQueryStep, TxExpect};

/// Attaches the `call` method to denali steps,
...
/// Attaches the `call_expect` method to denali steps,
...
fn process_contract_call<OriginalResult>(
    ...
    let denali_args = convert_call_args(&contract_call.arg_buffer);
    (to_str, function, denali_args)
}
