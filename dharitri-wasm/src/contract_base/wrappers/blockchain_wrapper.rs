    ...
    A: BlockchainApi + ManagedTypeApi + ErrorApi,
{
    ...
    pub fn get_current_dct_nft_nonce(
        ...
        A::blockchain_api_impl()
            .get_current_dct_nft_nonce(address.get_raw_handle(), token_id.get_raw_handle())
    }
    ...
    pub fn is_dct_paused(&self, token_id: &TokenIdentifier<A>) -> bool {
        A::blockchain_api_impl().check_dct_paused(token_id.get_raw_handle())
    }

    #[cfg(feature = "ei-1-2")]
    pub fn is_dct_limited_transfer(&self, token_id: &TokenIdentifier<A>) -> bool {
        A::blockchain_api_impl().check_dct_limited_transfer(token_id.get_raw_handle())
    }

    #[inline]
    pub fn get_dct_local_roles(&self, token_id: &TokenIdentifier<A>) -> DctLocalRoleFlags {
        A::blockchain_api_impl().load_dct_local_roles(token_id.get_raw_handle())
    }
}
    ...
    A: BlockchainApi + StorageReadApi + ManagedTypeApi + ErrorApi,
{
    ...
    pub fn get_cumulated_validator_rewards(&self) -> BigUint<A> {
        ...
        A::storage_read_api_impl()
            .storage_load_big_uint_raw(storage::protected_keys::DHARITRI_REWARD_KEY, result_handle);
        BigUint::from_raw_handle(result_handle)
    }
}
